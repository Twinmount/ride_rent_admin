...
// fetch all cities paginated
export const fetchAllCitiesPaginated = async ({
  page,
  limit,
  state,
  search,
  ...
  search?: string;
  sortOrder?: string;
}): Promise<FetchCitiesPaginatedResponse> => {
  try {
    ...
    const url = `${Slug.GET_ALL_CITIES_PAGINATED}?${queryParams.toString()}`;

    const data = await API.get<FetchCitiesPaginatedResponse>({
      slug: url,
    });
    if (!data) {
      throw new Error("Failed to fetch city data");
