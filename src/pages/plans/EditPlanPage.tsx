...
import { usePlan, usePlanMutations } from "@/hooks/usePlans";
...
export default function EditPlanPage() {
    ...
    const { data: plan, isLoading: fetchLoading } = usePlan(id!);

    const { updatePlanMutation, loading: updateLoading } = usePlanMutations();
    ...
    useEffect(() => {
        if (plan) {
            const hasT1 = Boolean(plan.T1);
            const hasT2 = Boolean(plan.T2);
            const hasT3 = Boolean(plan.T3);
            ...
            form.reset({
                planType: plan.planType || PlanType.LISTING,
                plan: plan.plan,
                ...
                enableT2: hasT2,
                enableT3: hasT3,
                T1: hasT1 ? (plan.T1 as any) : undefined,
                T2: hasT2 ? (plan.T2 as any) : undefined,
                T3: hasT3 ? (plan.T3 as any) : undefined,
            });
        }
    }, [plan, form]);
    ...
    const onSubmit = async (values: PlanFormValues) => {
        ...
        try {
            ...
            await updatePlanMutation.mutateAsync({ id: id!, payload });
            toast({
                title: "Success",
