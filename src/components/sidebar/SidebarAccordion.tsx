...
export default function SidebarAccordion({
  ...
  const hasAnySubItemsHasFeatureStatus = item.items?.some((i) =>
    Boolean(i.featureStatus),
  );
    ...
    >
      ...
      >
        <SidebarAccordionTrigger
          icon={item.icon}
          label={item.label}
          hasAnySubItemsHasFeatureStatus={hasAnySubItemsHasFeatureStatus}
        />
      </AccordionTrigger>
      <AccordionContent className="ml-auto mr-1 mt-1 flex !h-fit !max-h-fit !min-h-fit w-[89%] flex-col items-start justify-center rounded-r-lg border-l border-l-gray-400 bg-gray-100/50 pt-3">
        {item.items?.map((subItem) => (
          <AccordionSidebarItem
            key={subItem.link}
            item={subItem}
            isActive={isSubItemActive(subItem.link)}
            featureStatus={subItem.featureStatus}
          />
        ))}
      </AccordionContent>
    </AccordionItem>
...
type SidebarAccordionTriggerProps = {
  ...
  label: string;
  hasAnySubItemsHasFeatureStatus?: boolean;
};
...
const SidebarAccordionTrigger = ({
  icon,
  label,
  hasAnySubItemsHasFeatureStatus = false,
  ...
  return (
    <div className={`${classNames} `}>
      ...
      <span className="">
        {label}{" "}
        {hasAnySubItemsHasFeatureStatus && (
